...
```r
#There are a lot of useless variables as well variables with empty cells or NA
#I will remove those before moving further.
pmltrain <- pmltrain[,-(1:7)]
pmltrain <- pmltrain[,-grep("kurtosis|skewness|max|min|amplitude|var|avg|stddev",names(pmltrain))]
names(pmltrain)[53] <- "classes"
```
...
```r
#Split training set into training and test set. Actual downloaded test set will be used as validation set.
pmltrain$classes <- as.factor(pmltrain$classes)
set.seed(123)
split <- createDataPartition(pmltrain$classes,p=0.8,list=FALSE)
pmltrain1 <- pmltrain[split,]
pmltrain2 <- pmltrain[-split,]
```

##Step 4: Cross validation and model building


```r
#For cross validation, I will use 5 fold cross validation to see what the out of bag error is and tune the
#parameters for better results. 5 fold is also less computationally intense than 10 fold.
fitcontrol <- trainControl(method="cv",number=5)
#For model building, I would like to use Random Forest for more accurate results. I will use the train function
#from the caret package and under method, select rf as in random forest, feed the cv parameters in the train
#function and run the model. The algorithm takes around 30 minutes to run. Not only is this going to produce
#highly accurate results, but also not computationaly intense.
...
```r
train1 <- train(classes~.,data=pmltrain1,method="rf",trControl=fitcontrol)
train1
```
...
## 
##   mtry  Accuracy   Kappa    
##    2    0.9920378  0.9899275
##   27    0.9921016  0.9900084
##   52    0.9864322  0.9828356
## 
...
```r
train2 <- predict(train1$finalModel,pmltrain2[,-53])
confusionMatrix(train2,pmltrain2[,53])
```
...
## Sensitivity            0.9991   0.9868   0.9956   0.9922   0.9986
## Specificity            0.9971   0.9994   0.9978   0.9991   1.0000
## Pos Pred Value         0.9929   0.9973   0.9898   0.9953   1.0000
## Neg Pred Value         0.9996   0.9968   0.9991   0.9985   0.9997
## Prevalence             0.2845   0.1935   0.1744   0.1639   0.1838
## Detection Rate         0.2842   0.1909   0.1736   0.1626   0.1835
## Detection Prevalence   0.2863   0.1914   0.1754   0.1634   0.1835
## Balanced Accuracy      0.9981   0.9931   0.9967   0.9957   0.9993
```

```r
#Use rf model to predict the test set and the results show the OOS error expected is 0.46%. I am expecting
#similar error on the 20 cases from the validation set.
```

##Step 5: Predict on 20 different cases and submit results to project quiz


```r
#Transform validation set and feed back to final model to predict
summary(pmltest)
```
...
```r
pmltest <- pmltest[,-(1:7)]
pmltest <- pmltest[,-grep("kurtosis|skewness|max|min|amplitude|var|avg|stddev",names(pmltest))]
pmltest[,53] <- NULL
predict(train1$finalModel,pmltest)
```
